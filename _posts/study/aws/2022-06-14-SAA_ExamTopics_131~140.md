...
image:
  path: /assets/img/study_AWS/2022-06-12-SAA_ExamTopics_131~140/logo.png
---
...
A. 데이터를 Amazon S3에 복제하려면 구내에 AWS Storage Gateway 파일 게이트웨이를 설치합니다.

B. 데이터를 Amazon S3에 복제하려면 구내에 AWS Storage Gateway 파일 게이트웨이 하드웨어 장치를 설치합니다.

C. 저장된 볼륨이 있는 AWS Storage Gateway 볼륨 게이트웨이를 구내에 설치하여 데이터를 Amazon S3에 복제합니다.

D. 데이터를 Amazon S3에 복제하려면 사내에서 캐시된 볼륨이 있는 AWS Storage Gateway 볼륨 게이트웨이를 설치합니다.
...
C. AWS Transfer for SFTP 및 Amazon S3 버킷을 사용하여 SFTP 서버 끝점을 생성합니다. SFTP 서버에 연결하도록 자동 스케일링 그룹의 EC2 인스턴스를 구성합니다.
